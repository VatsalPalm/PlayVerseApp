...
const TournamentDetailsScreen = () => {
  ...
  const loadAllData = useCallback(async () => {
    ...
    try {
      ...
      const [tData, pData, fData, sData, aData] = await Promise.all([
        fetchTournamentControllerGetTournament({ pathParams: { id: tournamentId } }).catch(() => null) as Promise<any>,
        fetchTournamentControllerGetParticipants({ pathParams: { id: tournamentId } }).catch(() => []) as Promise<any>,
        fetchTournamentControllerGetFixtures({ pathParams: { id: tournamentId } }).catch(() => []) as Promise<any>,
        fetchTournamentControllerGetStandings({ pathParams: { id: tournamentId } }).catch(() => []) as Promise<any>,
        fetchTournamentControllerGetAnalytics({ pathParams: { id: tournamentId } }).catch(() => null) as Promise<any>,
      ]);
  ...
  const handleRegisterTeam = async () => {
    // We assume the user has a team. Let's prompt for Team ID.
    Alert.prompt(
      'Register Team',
      'Please enter your Team ID to register for this tournament:',
      async (text) => {
        const teamIdNum = parseInt(text, 10);
        if (isNaN(teamIdNum)) {
        ...
        try {
          ...
          showMessage({ message: 'Registration requested successfully!', type: 'success' });
          loadAllData();
        } catch (err: any) {
          showMessage({ message: err.message || 'Registration failed', type: 'danger' });
        } finally {
  ...
  const handleApproveTeam = async (teamId: number) => {
    try {
      ...
      showMessage({ message: err.message || 'Approval failed', type: 'danger' });
    } finally {
      setActionLoading(false);
    }
  };
  ...
  const handleCancelTeam = async (teamId: number) => {
    try {
      ...
      showMessage({ message: err.message || 'Cancellation failed', type: 'danger' });
    } finally {
      setActionLoading(false);
    }
  };
  ...
  const handleSaveSeeding = async () => {
    try {
      ...
      showMessage({ message: 'Seeding updated successfully!', type: 'success' });
      loadAllData();
    } catch (err: any) {
      showMessage({ message: err.message || 'Seeding update failed', type: 'danger' });
    } finally {
      setActionLoading(false);
    }
  };
  ...
  const handleSaveGroups = async () => {
    try {
      ...
      showMessage({ message: err.message || 'Groups update failed', type: 'danger' });
    } finally {
      setActionLoading(false);
    }
  };
  ...
  const handleGenerateBrackets = async () => {
    try {
      ...
      showMessage({ message: 'Brackets & fixtures generated!', type: 'success' });
      loadAllData();
    } catch (err: any) {
      showMessage({ message: err.message || 'Bracket generation failed', type: 'danger' });
    } finally {
      setActionLoading(false);
    }
  };
  ...
  const handleCreateManualMatch = async () => {
    if (!selectedHomeId || !selectedAwayId) {
      showMessage({ message: 'Please select both home and away teams', type: 'warning' });
      return;
    }
    if (selectedHomeId === selectedAwayId) {
      showMessage({ message: 'Home and Away teams must be different', type: 'warning' });
      return;
    }
    ...
    try {
      ...
      showMessage({ message: 'Manual match created successfully!', type: 'success' });
      setSelectedHomeId(null);
      setSelectedAwayId(null);
      loadAllData();
    } catch (err: any) {
      showMessage({ message: err.message || 'Failed to create manual match', type: 'danger' });
    } finally {
      setActionLoading(false);
    }
  };
    ...
    Alert.prompt(
      ...
      async (text) => {
        ...
        try {
          ...
          showMessage({ message: err.message || 'Match completion override failed', type: 'danger' });
        } finally {
          setActionLoading(false);
        }
      },
  ...
  const renderInfoTab = () => {
    ...
    return (
      <ScrollView contentContainerStyle={[styles.tabScroll, { paddingBottom: insets.bottom + 40 }]} showsVerticalScrollIndicator={false}>
        
        {/* Hero Banner */}
        <View style={styles.heroBanner}>
          <View style={styles.heroGlassHighlight} />
          <View style={styles.heroTopRow}>
            <View style={[styles.heroBadge, { backgroundColor: statusColor + '20', borderColor: statusColor + '50' }]}>
              <View style={[styles.heroBadgeDot, { backgroundColor: statusColor }]} />
              <Text style={[styles.heroBadgeText, { color: statusColor }]}>{tournament.status}</Text>
            </View>
            <View style={[styles.heroBadge, { backgroundColor: 'rgba(108,77,246,0.15)', borderColor: 'rgba(108,77,246,0.3)' }]}>
              <Text style={[styles.heroBadgeText, { color: '#6C4DF6' }]}>{tournament.format}</Text>
            </View>
          </View>
          <Text style={styles.heroTitle}>{tournament.name}</Text>
          <Text style={styles.heroSubtitle}>Organised by {tournament.organizer_name || 'System'}</Text>
        </View>
        ...
        <View style={styles.infoGrid}>
          <View style={styles.infoGridCard}>
            ...
            <Text style={styles.infoGridVal}>
              {tournament.start_date ? new Date(tournament.start_date).toLocaleDateString('en-IN', { day: 'numeric', month: 'short', year: 'numeric' }) : 'TBD'}
            </Text>
          </View>
          <View style={styles.infoGridCard}>
            <Text style={styles.infoGridIcon}>🏁</Text>
            ...
            <Text style={styles.infoGridVal}>
              {tournament.end_date ? new Date(tournament.end_date).toLocaleDateString('en-IN', { day: 'numeric', month: 'short', year: 'numeric' }) : 'TBD'}
            </Text>
          </View>
          <View style={styles.infoGridCard}>
            <Text style={styles.infoGridIcon}>👥</Text>
            <Text style={styles.infoGridLabel}>Max Teams</Text>
            <Text style={styles.infoGridVal}>{tournament.config?.maxTeams || tournament.max_teams || 16}</Text>
          </View>
          <View style={styles.infoGridCard}>
            <Text style={styles.infoGridIcon}>🔒</Text>
            <Text style={styles.infoGridLabel}>Visibility</Text>
            <Text style={styles.infoGridVal}>{tournament.visibility || 'PUBLIC'}</Text>
          </View>
        </View>
          ...
          <View style={styles.infoRow}>
            ...
            <View style={{ flex: 1 }}>
              ...
              <Text style={styles.infoRowVal}>
                {new Date(tournament.config.registrationDeadline).toLocaleString('en-IN', { day: 'numeric', month: 'short', year: 'numeric', hour: '2-digit', minute: '2-digit' })}
              </Text>
            </View>
          </View>
        )}
        ...
        {!isOrganizer && tournament.status === 'UPCOMING' && (
          <TouchableOpacity style={styles.primaryBtn} onPress={handleRegisterTeam} activeOpacity={0.8}>
  ...
  const renderParticipantsTab = () => {
    return (
      <ScrollView contentContainerStyle={[styles.tabScroll, { paddingBottom: insets.bottom + 40 }]} showsVerticalScrollIndicator={false}>
        <Text style={styles.sectionHeader}>Registered Teams ({participants.length})</Text>
            ...
            <View key={index} style={styles.participantItem}>
              <View style={{ flex: 1 }}>
                ...
                <Text style={styles.participantDetails}>
                  Status: {item.status} {item.seed ? `• Seed: ${item.seed}` : ''}{' '}
              ...
              {item.status === 'PENDING' && (
                <View style={styles.actionRow}>
                  <TouchableOpacity style={styles.checkBtn} onPress={() => handleApproveTeam(item.team_id)}>
                    <Ionicons name="checkmark-circle" size={24} color="#00E676" />
                  </TouchableOpacity>
                  <TouchableOpacity style={styles.checkBtn} onPress={() => handleCancelTeam(item.team_id)}>
        ...
        {participants.length > 0 && (
          <View style={styles.organizerTools}>
            <Text style={styles.toolsTitle}>Seeding & Groupings (Organizer Only)</Text>
            ...
            {participants.map((item, index) => (
              <View key={index} style={styles.seedRow}>
                ...
                <TextInput
                  ...
                  onChangeText={(val) => setSeedsInput((prev) => ({ ...prev, [item.team_id]: val }))}
                />
                <TextInput
                  style={styles.seedInput}
                  placeholder="Group"
                  ...
                  onChangeText={(val) => setGroupsInput((prev) => ({ ...prev, [item.team_id]: val }))}
                />
              </View>
            ))}

            <View style={{ flexDirection: 'row', gap: 10, marginTop: 12 }}>
              <TouchableOpacity style={[styles.primaryBtn, { flex: 1 }]} onPress={handleSaveSeeding}>
                <Text style={styles.primaryBtnText}>Save Seeds</Text>
              </TouchableOpacity>
              <TouchableOpacity style={[styles.primaryBtn, { flex: 1, backgroundColor: '#00D2FF' }]} onPress={handleSaveGroups}>
            ...
            {fixtures.length === 0 && (
              <View style={styles.bracketGenCard}>
                <Text style={styles.toolsTitle}>Generate Brackets & Matches</Text>
                <SizedBox height={10} />
                ...
                />
                <SizedBox height={16} />
                <TouchableOpacity style={[styles.primaryBtn, { backgroundColor: '#A855F7' }]} onPress={handleGenerateBrackets}>
  ...
  const renderFixturesTab = () => {
    return (
      <ScrollView contentContainerStyle={[styles.tabScroll, { paddingBottom: insets.bottom + 40 }]} showsVerticalScrollIndicator={false}>
        <Text style={styles.sectionHeader}>Fixtures & Brackets ({fixtures.length})</Text>
          ...
          fixtures.map((item, index) => (
            <View key={index} style={styles.fixtureCard}>
              <View style={styles.fixtureHeader}>
                <Text style={styles.fixtureRound}>Match {index + 1} ({item.match_type || 'SINGLES'})</Text>
                <View
                  style={[
                    styles.statusBadge,
                    {
                      borderColor: (statusColors[item.status] || '#FFF') + '40',
                      backgroundColor: (statusColors[item.status] || '#FFF') + '15',
                    },
                  ]}
                >
                  <Text style={[styles.statusText, { color: statusColors[item.status] || '#FFF' }]}>{item.status}</Text>
                </View>
              </View>

              <View style={styles.teamsRow}>
                <Text style={[styles.teamNameText, item.winner_team_id === item.home_team_id && styles.winnerBold]}>
                  {item.home_team_name || 'TBD'}
                </Text>
                <Text style={styles.vsText}>VS</Text>
                <Text style={[styles.teamNameText, item.winner_team_id === item.away_team_id && styles.winnerBold]}>
                  {item.away_team_name || 'TBD'}
                </Text>
              </View>

              <Text style={styles.fixtureDate}>
                {item.scheduled_at ? new Date(item.scheduled_at).toLocaleString() : 'Date TBD'}
              </Text>
              ...
              <View style={styles.fixtureActions}>
                {item.status !== 'COMPLETED' && (
                  <TouchableOpacity
                    ...
                    onPress={() => navigation.navigate('LiveScoring', { matchId: item.id, canScore: true })}
                  >
        ...
        {participants.length >= 2 && (
          <View style={[styles.bracketGenCard, { marginTop: 24, marginBottom: 20 }]}>
            ...
            >
              <Text style={styles.teamSelectBtnText}>
                {participants.find(p => p.team_id === selectedHomeId)?.team_name || 'Select Home Team'}
              </Text>
            </TouchableOpacity>
            ...
            >
              <Text style={styles.teamSelectBtnText}>
                {participants.find(p => p.team_id === selectedAwayId)?.team_name || 'Select Away Team'}
              </Text>
            </TouchableOpacity>

            <SizedBox height={12} />

            <Text style={styles.infoLabel}>Scheduled Date & Time (YYYY-MM-DD HH:mm)</Text>
            <TextInput
              style={styles.formInput}
          ...
          <View style={styles.tableCard}>
            ...
            {standings.map((item: any, index: number) => (
              <View key={index} style={styles.tableRow}>
                <Text style={[styles.tableText, { flex: 0.8 }]}>{index + 1}</Text>
                <Text style={[styles.tableText, { flex: 3, fontWeight: '700' }]} numberOfLines={1}>
                  {item.teamName || 'Team'}
                </Text>
                <Text style={[styles.tableText, { flex: 1 }]}>{item.played}</Text>
                <Text style={[styles.tableText, { flex: 1 }]}>{item.wins}</Text>
                <Text style={[styles.tableText, { flex: 1 }]}>{item.losses}</Text>
                <Text style={[styles.tableText, { flex: 1, color: '#00E676', fontWeight: '800' }]}>{item.points}</Text>
              </View>
            ))}
          </View>
        )}
      ...
      <ScrollView contentContainerStyle={[styles.tabScroll, { paddingBottom: insets.bottom + 40 }]} showsVerticalScrollIndicator={false}>
        ...
        <View style={styles.analyticsGrid}>
          ...
          <View style={styles.analyticsCard}>
            ...
            <Text style={styles.analyticsVal}>{analytics.completedMatches || 0}</Text>
          </View>
          <View style={styles.analyticsCard}>
            <Text style={styles.analyticsLabel}>Average Match Duration</Text>
            <Text style={styles.analyticsVal}>{analytics.avgDurationMinutes || 0} min</Text>
          </View>
        </View>
      </ScrollView>
    );
      ...
      <Modal visible={showHomePicker} transparent animationType="slide">
        <View style={styles.modalOverlay}>
          <View style={styles.modalContent}>
            ...
            <TouchableOpacity style={styles.modalCloseBtn} onPress={() => setShowHomePicker(false)}>
      ...
      <Modal visible={showAwayPicker} transparent animationType="slide">
        <View style={styles.modalOverlay}>
          <View style={styles.modalContent}>
            ...
            <TouchableOpacity style={styles.modalCloseBtn} onPress={() => setShowAwayPicker(false)}>
              <Text style={styles.modalCloseText}>Cancel</Text>
            </TouchableOpacity>
          </View>
        </View>
